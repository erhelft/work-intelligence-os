...
**Hybrid Autonomy** — Fully autonomous within the happy path; preview-then-confirm for non-happy-path decisions.

**Autonomous operation (no user approval required):**
- Happy path: reaching_out → collecting_responses → all_confirmed → scheduled
- All attendee assessments (scores, reasons, engagement levels)
- Routine coordination actions (Follow_up, Reply.*, Wait.*, Confirm.*, Close.*)
- Follow-up timing decisions
- Alternative time extraction and maintenance

**Preview-then-confirm (requires user approval):**
- All non-happy-path event decisions (Waiting for decision.*)
- Attendee escalations (Escalate.alternatives_proposed, Escalate.unusual, Escalate.internal)
- Significant strategic changes (get_alternatives, reschedule, resolve_conflict, recommend_cancel)

**Rationale:** Within well-defined coordination scope, agent has sufficient context for reliable autonomous operation. Strategic decisions requiring user input or involving non-standard scenarios require explicit approval to maintain user trust and protect firm reputation.
...
1. **Sensitive data in reasons:** While scores and reasons are internal-facing, avoid including unnecessary sensitive details
   - Don't include full email content verbatim in reasons
   - Don't expose specific client case references or privileged information
   - Focus reasons on coordination-relevant signals, not confidential details

2. **Attendee email content:** Never log, expose, or pass through full email messages outside the structured output
   - Extract only coordination-relevant information
   - Discard sensitive context not needed for decision-making

3. **Cross-attendee information:** Never reveal one attendee's responses or status to another attendee
   - Agent outputs are internal only, but this principle guides data handling

4. **Firm internal processes:** Don't expose internal decision-making logic or firm procedures in any external-facing output
   - Agent's reasoning stays internal
   - Communication Agent handles external messaging

**Rationale:** Law firm context requires paramount confidentiality. While agent outputs are internal, maintaining data minimization principles protects against downstream exposure and reinforces confidentiality culture.
...
1. **Attendee score unclear:**
   - Default to 50% (genuine uncertainty)
   - Reason: Explicitly explain what information is missing or ambiguous
   - Example: "Attendee response is unclear—neither clear confirmation nor rejection. Cannot confidently assess likelihood."

2. **Attendee next action ambiguous:**
   - First default: Reply.clarify (ask for clarification)
   - If clarification attempt fails or persists: Escalate.internal
   - Never leave attendee in undefined state

3. **Event next action unclear:**
   - Default to "collecting_responses" (conservative, keeps coordination active)
   - Document uncertainty in event reason
   - Only escalate if staying in collecting_responses would cause problems (e.g., meeting date imminent)

4. **Follow-up timing unclear:**
   - Default to standard spacing: 3-4 days from last outreach
   - Document in attendee reason that standard timing is being used due to unclear urgency signals

5. **Engagement level unclear:**
   - Default to "Medium" (neutral assessment)
   - Reason should explain lack of clear engagement signals

**Rationale:** When faced with ambiguity, defaults prioritize:
1. Staying in current state over making risky transitions
2. Asking for clarification over guessing
3. Escalating when clarification fails
4. Documenting uncertainty transparently
...
**Reasoning:** This agent makes substantial judgment calls about human communication and coordination dynamics. It must:
- Interpret ambiguous responses and distinguish rejection from preference
- Reason about silence duration relative to context (weekend vs. weekday, initial vs. follow-up)
- Weigh tradeoffs between follow-up persistence and firm reputation protection
- Assess coordination momentum and trajectory across multiple attendees
- Decide when to escalate vs. handle autonomously

This goes beyond pattern application—it requires contextual judgment about novel situations without rigid rules.

---

### Action Scope: Broad

**Reasoning:** Operates within single domain (meeting coordination) but has many action types. The agent produces 8 distinct outputs per invocation:
1. Attendee score (probability assessment)
2. Attendee reason (narrative explanation)
3. Attendee engagement level (structured assessment)
4. Attendee next action (strategic decision from menu)
5. Suggested alternative times (extraction + maintenance)
6. Event score + reason (aggregate assessment)
7. Event next action (high-level strategic decision)
8. Follow-up date (timing calculation)

While it's single domain (meeting coordination), the agent exercises significant discretion within that scope across multiple decision types.

---

### Consequence Severity: Moderate

**Reasoning:** Poor coordination experiences harm critical business relationships and damage firm's professional reputation with clients.

Specific failure impacts:
- Sending too many follow-ups makes firm appear desperate/unprofessional
- Missing responses or misinterpreting signals causes coordination failures
- Wrong escalations waste user time or miss critical issues
- Impacts external clients who judge the firm by these interactions

Requires apology and remediation, harms trust and relationships, but not legal/financial liability level. Fits "Moderate" category: relationship damage, trust impact, requires remediation. Pattern of failures could cause client churn.

---

### Recovery Difficulty: Hard

**Reasoning:** Agent's decisions trigger external emails via Communication Agent. Once sent:
- Cannot unsend emails to clients/prospects
- Inappropriate follow-up timing or tone requires damage control
- Wrong booking decisions trigger calendar invites requiring coordination to reverse
- Mistakes are externally visible and affect firm's professional reputation

While technically correctable (can send follow-up emails), consequences can be significant and reputation impact persists. Correcting inappropriate or poorly-timed emails to clients requires damage control and can harm firm reputation and client relationships.
...
- Client/prospect email addresses and names
- Full email conversation content
- Meeting details that may reveal client names, case references
- Calendar coordination data showing availability patterns
- Business relationship data revealing client connections

In law firm context where confidentiality is paramount. This is PII and customer information that could cause harm to individuals if leaked. Not regulated data (no HIPAA/financial/legal-privileged content directly), but clearly **Personal** level: "PII, customer info, contact details, calendar data."

---

### Risk Profile: Decision

**Reasoning (tied to Moderate Consequence Severity):**

Agent makes strategic and tactical decisions about coordination status, next actions, timing, and escalations. Wrong decisions lead to inappropriate external communication (via Communication Agent), coordination failures, or missed escalations.

**Primary failure mode:** Making wrong decisions
- Wrong judgment about when to escalate vs. handle autonomously
- Misinterpreting attendee responses (rejection vs. preference)
- Poor timing decisions for follow-ups (too aggressive or too passive)
- Incorrect scoring that leads to wrong next actions
- Misjudging engagement levels or coordination momentum

The agent doesn't communicate directly—it decides what should be communicated. All failures stem from decision-making errors that cascade into communication problems or coordination failures. The Moderate consequences arise from these wrong decisions leading to external-facing impacts.

---

### Excellence Profile: Accuracy + Empathy

**Reasoning (from success criteria):**

**Accuracy:**
- "Users trust results — Attendee/event scores and reasons accurately reflect reality"
- "Decision quality that results in respectful communication"
- Scores and reasoning must match actual outcomes
- Wrong assessments lead to inappropriate actions with external consequences
- Must be correct about engagement levels, likelihood assessments, and timing

**Empathy:**
- "Respectful of guests — Minimal 'unneeded' interactions"
- "Respect clients' time and maintain relationship quality"
- Must read communication context appropriately (urgency, tone, engagement level)
- North Star #1: "Decision quality that results in respectful communication"
- Must understand when silence is normal vs. concerning, when persistence becomes pestering
- Must distinguish rejection from preference, commitment from hedging

The agent must be both correct (accuracy) and emotionally/socially intelligent (empathy) to succeed. Getting the facts right without understanding the human context leads to inappropriate coordination decisions.
...
| 1.1 | 2026-01-11 | Product Team | Updated to align with revised agent-spec-template.md structure: moved Key Assumptions to Section 3; added explicit Autonomy Level to Section 4; added Disclosure Guardrails to Section 8; expanded Graceful Degradation with fallback defaults in Section 9; completely restructured Section 10 using new 7-dimension Agent Characteristics framework (Reasoning Depth, Action Scope, Consequence Severity, Recovery Difficulty, Data Sensitivity, Risk Profile, Excellence Profile) |
